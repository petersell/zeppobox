<?xml version='1.0'?>
    ...
    <rx:meta-info>
      ...
      <xsl:if test="exists($title)">
        <rx:meta-field name="title" value="{$title}"/>
         ...
         <rx:meta-field name="title" value="Das Petersilienbuch"/>
         <rx:meta-field name="subject" value="Das Buch als Dienstleistung"/>
         <rx:meta-field name="keywords" value="EPUB, eBook, Publishing, Verlag, Content, Paid Content"/>
   ...
   <!-- external-graphic von front-matter.xsl --> 
        <fo:block text-align="center" width="100%">
          <fo:external-graphic src="url({concat($artworkPrefix, 'Customization/OpenTopic/common/artwork/artikelbild-2.gif')})"/>              
        </fo:block>
